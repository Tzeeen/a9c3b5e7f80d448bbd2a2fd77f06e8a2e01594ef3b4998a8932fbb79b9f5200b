...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const elementsToAnimate = document.querySelectorAll('h2, h3, p, ul, .feature-card, .stat-card, .related-article');
    ...
    const sampleComments = [
        {
            author: "Michael Tan",
            date: "July 23, 2025",
            text: "As a resident of Singapore, I can attest to how much smart technology has improved daily life. My commute is faster, utilities are more efficient, and public services are incredibly responsive."
        },
        {
            author: "Sarah Johnson",
            date: "July 22, 2025",
            text: "I worry about the privacy implications. All this data collection could be misused if proper safeguards aren't in place. We need strong regulations alongside the technological advances."
        },
        {
            author: "Raj Patel",
            date: "July 22, 2025",
            text: "The environmental benefits alone make smart cities worth pursuing. If we can reduce energy consumption and emissions while improving quality of life, it's a win-win for everyone."
        }
    ];
    ...
    const savedPoll = localStorage.getItem('smartCityPoll');
    if (savedPoll) {
...
function submitPoll() {
    ...
    const selectedOption = document.querySelector('input[name="smartCity"]:checked');
...
document.querySelectorAll('h2, h3, p, ul, .feature-card, .stat-card, .related-article').forEach(el => {
    observer.observe(el);
});
